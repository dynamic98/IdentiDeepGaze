...
class Study2AnalysisStimuli:
    ...
    def targetDistance(self, stimuliNum, participant, blockName, visual_component, level):
        thisDataframe = self.takeGaze(stimuliNum, participant, blockName)
        gazeX, gazeY = get_gazeXY(thisDataframe)
        gazeSize = len(gazeX)
        distanceList = []
        stimuliInfo = self.LevelDictionary[str(stimuliNum)]

        if self.stimuli == 'similar':
            
            pass
        elif self.stimuli == 'different':
            pass
